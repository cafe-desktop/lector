...
ev_annotation_properties_dialog_constructed (GObject *object)
{
	...
	case EV_ANNOTATION_TYPE_TEXT:
		label = ctk_label_new (_("Icon:"));
		ctk_label_set_xalign (GTK_LABEL (label), 0.0);
		ctk_grid_attach (GTK_GRID (grid), label, 0, 5, 1, 1);
		ctk_widget_show (label);

		dialog->icon = ctk_combo_box_text_new ();
		ctk_combo_box_text_append_text (GTK_COMBO_BOX_TEXT (dialog->icon), _("Note"));
		ctk_combo_box_text_append_text (GTK_COMBO_BOX_TEXT (dialog->icon), _("Comment"));
		ctk_combo_box_text_append_text (GTK_COMBO_BOX_TEXT (dialog->icon), _("Key"));
		ctk_combo_box_text_append_text (GTK_COMBO_BOX_TEXT (dialog->icon), _("Help"));
		ctk_combo_box_text_append_text (GTK_COMBO_BOX_TEXT (dialog->icon), _("New Paragraph"));
		ctk_combo_box_text_append_text (GTK_COMBO_BOX_TEXT (dialog->icon), _("Paragraph"));
		ctk_combo_box_text_append_text (GTK_COMBO_BOX_TEXT (dialog->icon), _("Insert"));
		ctk_combo_box_text_append_text (GTK_COMBO_BOX_TEXT (dialog->icon), _("Cross"));
		ctk_combo_box_text_append_text (GTK_COMBO_BOX_TEXT (dialog->icon), _("Circle"));
		ctk_combo_box_text_append_text (GTK_COMBO_BOX_TEXT (dialog->icon), _("Unknown"));
		ctk_combo_box_set_active (GTK_COMBO_BOX (dialog->icon), 0);
		ctk_grid_attach (GTK_GRID (grid), dialog->icon, 1, 5, 1, 1);
		ctk_widget_set_hexpand (dialog->icon, TRUE);
		ctk_widget_show (dialog->icon);
...
ev_annotation_properties_dialog_init (EvAnnotationPropertiesDialog *annot_dialog)
{
	...
	ctk_window_set_title (GTK_WINDOW (annot_dialog), _("Annotation Properties"));
	ctk_window_set_destroy_with_parent (GTK_WINDOW (annot_dialog), TRUE);
	ctk_container_set_border_width (GTK_CONTAINER (annot_dialog), 5);
	ctk_dialog_add_buttons (dialog,
				"ctk-close", GTK_RESPONSE_CLOSE,
				"ctk-apply", GTK_RESPONSE_APPLY,
				NULL);
	ctk_dialog_set_default_response (dialog, GTK_RESPONSE_APPLY);
	...
	ctk_box_pack_start (GTK_BOX (content_area), grid, FALSE, FALSE, 0);
	ctk_widget_show (grid);

	label = ctk_label_new (_("Author:"));
	ctk_label_set_xalign (GTK_LABEL (label), 0.0);
	ctk_grid_attach (GTK_GRID (grid), label, 0, 0, 1, 1);
	ctk_widget_show (label);

	annot_dialog->author = ctk_entry_new ();
	ctk_entry_set_text (GTK_ENTRY (annot_dialog->author), g_get_real_name ());
	ctk_grid_attach (GTK_GRID (grid), annot_dialog->author, 1, 0, 1, 1);
	ctk_widget_set_hexpand (annot_dialog->author, TRUE);
	ctk_widget_show (annot_dialog->author);

	label = ctk_label_new (_("Color:"));
	ctk_label_set_xalign (GTK_LABEL (label), 0.0);
	ctk_grid_attach (GTK_GRID (grid), label, 0, 1, 1, 1);
	ctk_widget_show (label);

	annot_dialog->color = ctk_color_button_new_with_rgba (&yellow);
	ctk_grid_attach (GTK_GRID (grid), annot_dialog->color, 1, 1, 1, 1);
	ctk_widget_set_hexpand (annot_dialog->color, TRUE);
	ctk_widget_show (annot_dialog->color);

	label = ctk_label_new (_("Style:"));
	ctk_label_set_xalign (GTK_LABEL (label), 0.0);
	ctk_grid_attach (GTK_GRID (grid), label, 0, 2, 1, 1);
	ctk_widget_show (label);

	annot_dialog->opacity = ctk_scale_new_with_range (GTK_ORIENTATION_HORIZONTAL, 0, 100, 5);
	ctk_range_set_value (GTK_RANGE (annot_dialog->opacity), 100);
	ctk_grid_attach (GTK_GRID (grid), annot_dialog->opacity, 1, 2, 1, 1);
	ctk_widget_set_hexpand (annot_dialog->opacity, TRUE);
	ctk_widget_show (annot_dialog->opacity);
	...
	ctk_grid_attach (GTK_GRID (grid), hbox, 1, 3, 1, 1);
	ctk_widget_set_hexpand (hbox, TRUE);
	ctk_widget_show (hbox);

	label = ctk_label_new (_("Initial window state:"));
	ctk_label_set_xalign (GTK_LABEL (label), 0.0);
	ctk_grid_attach (GTK_GRID (grid), label, 0, 4, 1, 1);
	ctk_widget_show (label);

	annot_dialog->popup_state = ctk_combo_box_text_new ();
	ctk_combo_box_text_append_text (GTK_COMBO_BOX_TEXT (annot_dialog->popup_state), _("Open"));
	ctk_combo_box_text_append_text (GTK_COMBO_BOX_TEXT (annot_dialog->popup_state), _("Close"));
	ctk_combo_box_set_active (GTK_COMBO_BOX (annot_dialog->popup_state), 1);
	ctk_grid_attach (GTK_GRID (grid), annot_dialog->popup_state, 1, 4, 1, 1);
	ctk_widget_set_hexpand (annot_dialog->popup_state, TRUE);
	ctk_widget_show (annot_dialog->popup_state);
}
...
ev_annotation_properties_dialog_new_with_annotation (EvAnnotation *annot)
{
	...
	if (label)
		ctk_entry_set_text (GTK_ENTRY (dialog->author), label);

	ev_annotation_get_rgba (annot, &rgba);
	ctk_color_chooser_set_rgba (GTK_COLOR_CHOOSER (dialog->color), &rgba);

	opacity = ev_annotation_markup_get_opacity (EV_ANNOTATION_MARKUP (annot));
	ctk_range_set_value (GTK_RANGE (dialog->opacity), opacity * 100);

	is_open = ev_annotation_markup_get_popup_is_open (EV_ANNOTATION_MARKUP (annot));
	ctk_combo_box_set_active (GTK_COMBO_BOX (dialog->popup_state),
				  is_open ? 0 : 1);

	if (EV_IS_ANNOTATION_TEXT (annot)) {
		EvAnnotationText *annot_text = EV_ANNOTATION_TEXT (annot);

		ctk_combo_box_set_active (GTK_COMBO_BOX (dialog->icon),
...
{
	return ctk_entry_get_text (GTK_ENTRY (dialog->author));
}
