...
static void       ev_view_size_request_single_page           (EvView             *view,
							      CtkRequisition     *requisition);
static void       ev_view_size_request                       (CtkWidget          *widget,
							      CtkRequisition     *requisition);
...
static AtkObject *ev_view_get_accessible                     (CtkWidget *widget);
...
static void       ev_view_primary_get_cb                     (CtkClipboard       *clipboard,
							      CtkSelectionData   *selection_data,
							      guint               info,
							      gpointer            data);
static void       ev_view_primary_clear_cb                   (CtkClipboard       *clipboard,
							      gpointer            data);
...
G_DEFINE_TYPE_WITH_CODE (EvView, ev_view, CTK_TYPE_CONTAINER,
			    ...
			    CtkOrientation orientation)
{
	...
	CtkWidget *sb;
	CtkWidget *swindow = ctk_widget_get_parent (CTK_WIDGET (view));
	CtkAllocation allocation;
	CtkRequisition req;
	gint spacing;

	if (!CTK_IS_SCROLLED_WINDOW (swindow))
		return 0;
	...
	if (orientation == CTK_ORIENTATION_VERTICAL) {
		if (allocation.height >= view->requisition.height)
			sb = ctk_scrolled_window_get_vscrollbar (CTK_SCROLLED_WINDOW (swindow));
		else
			return 0;
	} else {
		if (allocation.width >= view->requisition.width)
			sb = ctk_scrolled_window_get_hscrollbar (CTK_SCROLLED_WINDOW (swindow));
		else
			return 0;
	}

	ctk_widget_style_get (swindow, "scrollbar_spacing", &spacing, NULL);
	ctk_widget_get_preferred_size (sb, &req, NULL);

	return (orientation == CTK_ORIENTATION_VERTICAL ? req.width : req.height) + spacing;
}
		 ...
		 CtkOrientation orientation)
{
	...
	if (orientation == CTK_ORIENTATION_VERTICAL) {
		page_size = ctk_adjustment_get_page_size (view->vadjustment);
		upper = ctk_adjustment_get_upper (view->vadjustment);
		lower = ctk_adjustment_get_lower (view->vadjustment);

		if (view->continuous) {
    			ctk_adjustment_clamp_page (view->vadjustment,
						   y - view->spacing / 2,
						   y + page_size);
		} else {
			ctk_adjustment_set_value (view->vadjustment,
						  CLAMP (y, lower, upper - page_size));
		}
	} else {
		page_size = ctk_adjustment_get_page_size (view->hadjustment);
		upper = ctk_adjustment_get_upper (view->hadjustment);
		lower = ctk_adjustment_get_lower (view->hadjustment);

		if (is_dual_page (view, NULL)) {
			ctk_adjustment_clamp_page (view->hadjustment, x,
						   x + page_size);
		} else {
			ctk_adjustment_set_value (view->hadjustment,
...
static void
ev_view_scroll_to_page_position (EvView *view, CtkOrientation orientation)
{
	gdouble x, y;
			    ...
			    CtkOrientation  orientation)
{
	...
	ctk_adjustment_set_page_size (adjustment, page_size);
	ctk_adjustment_set_step_increment (adjustment, alloc_size * 0.1);
	ctk_adjustment_set_page_increment (adjustment, alloc_size * 0.9);
	ctk_adjustment_set_lower (adjustment, 0);
	ctk_adjustment_set_upper (adjustment, upper);
	...
	switch (view->pending_scroll) {
			...
			ctk_adjustment_set_value (adjustment, (int)new_value);
			break;
    	        case SCROLL_TO_PAGE_POSITION:
...
view_update_range_and_current_page (EvView *view)
{
	...
	if (view->continuous) {
		...
		current_area.x = ctk_adjustment_get_value (view->hadjustment);
		current_area.width = ctk_adjustment_get_page_size (view->hadjustment);
		current_area.y = ctk_adjustment_get_value (view->vadjustment);
		current_area.height = ctk_adjustment_get_page_size (view->vadjustment);
			   ...
			   CtkOrientation  orientation)
{
	...
	ctk_binding_entry_add_signal (binding_set, keyval, modifiers,
				      ...
				      CTK_TYPE_ORIENTATION, orientation);
	ctk_binding_entry_add_signal (binding_set, keypad_keyval, modifiers,
				      "scroll", 2,
				      CTK_TYPE_SCROLL_TYPE, scroll,
				      CTK_TYPE_ORIENTATION, orientation);
}
...
{
	CtkWidget *widget = CTK_WIDGET (view);
	CtkAdjustment *adjustment = view->vadjustment;
	...
	if (scroll != CTK_SCROLL_PAGE_BACKWARD && scroll != CTK_SCROLL_PAGE_FORWARD)
		return ctk_adjustment_get_page_size (adjustment);

	page = scroll == CTK_SCROLL_PAGE_BACKWARD ? view->start_page : view->end_page;
	...
	rect.y = view->scroll_y + (scroll == CTK_SCROLL_PAGE_BACKWARD ? 5 : allocation.height - 5);
	...
	if (cairo_region_num_rectangles (region)) {
		...
		if (cairo_region_num_rectangles (sel_region) > 0) {
			...
			fraction = 1 - (rect.height / ctk_adjustment_get_page_size (adjustment));
		}
		cairo_region_destroy (sel_region);
	}
	cairo_region_destroy (region);

	return ctk_adjustment_get_page_size (adjustment) * fraction;
		...
		gboolean       horizontal)
{
	...
	if ((!horizontal && ev_view_page_fits (view, CTK_ORIENTATION_VERTICAL)) ||
	    (horizontal && ev_view_page_fits (view, CTK_ORIENTATION_HORIZONTAL))) {
		switch (scroll) {
	...
	page_size = ctk_adjustment_get_page_size (adjustment);
	step_increment = ctk_adjustment_get_step_increment (adjustment);
	...
	switch (scroll) {
		case CTK_SCROLL_PAGE_BACKWARD:
			...
			if (value == lower && first_page) {
				...
				increment = compute_scroll_increment (view, CTK_SCROLL_PAGE_BACKWARD);
		...
		case CTK_SCROLL_PAGE_FORWARD:
			...
			/* Jump to the bottom */
				...
				increment = compute_scroll_increment (view, CTK_SCROLL_PAGE_FORWARD);
...
ev_view_scroll_internal (EvView        *view,
			 ...
			 CtkOrientation orientation)
{
	ev_view_scroll (view, scroll, orientation == CTK_ORIENTATION_HORIZONTAL);
}
...
ensure_rectangle_is_visible (EvView *view, CdkRectangle *rect)
{
	...
	adjustment = view->vadjustment;
	adj_value = ctk_adjustment_get_value (adjustment);

	if (rect->y < adj_value) {
		value = MAX (ctk_adjustment_get_lower (adjustment), rect->y - MARGIN);
		ctk_adjustment_set_value (view->vadjustment, value);
	} else if (rect->y + rect->height > adj_value + allocation.height) {
		value = MIN (ctk_adjustment_get_upper (adjustment), rect->y + rect->height -
	...
	if (rect->x < adj_value) {
		value = MAX (ctk_adjustment_get_lower (adjustment), rect->x - MARGIN);
		ctk_adjustment_set_value (view->hadjustment, value);
	} else if (rect->x + rect->height > adj_value + allocation.width) {
		value = MIN (ctk_adjustment_get_upper (adjustment), rect->x + rect->width -
...
static void
compute_border (EvView *view, CtkBorder *border)
{
	CtkWidget       *widget = CTK_WIDGET (view);
	...
	ctk_style_context_add_class (context, EV_STYLE_CLASS_DOCUMENT_PAGE);
	     ...
	     EvRectangle *doc_rect)
{
	...
	ctk_widget_set_parent (child_widget, CTK_WIDGET (view));
...
static void
ev_view_form_field_text_changed (CtkWidget   *widget,
				 EvFormField *field)
{
	EvFormFieldText *field_text = EV_FORM_FIELD_TEXT (field);
	gchar           *text = NULL;
	...
	if (CTK_IS_ENTRY (widget)) {
		text = g_strdup (ctk_entry_get_text (CTK_ENTRY (widget)));
	} else if (CTK_IS_TEXT_BUFFER (widget)) {
		CtkTextIter start, end;

		ctk_text_buffer_get_bounds (CTK_TEXT_BUFFER (widget), &start, &end);
		text = ctk_text_buffer_get_text (CTK_TEXT_BUFFER (widget),
						 &start, &end, FALSE);
	}
				       ...
				       EvFormField *field)
{
	...
	switch (field_text->type) {
			...
			text = ctk_entry_new ();
			ctk_entry_set_has_frame (CTK_ENTRY (text), FALSE);
			ctk_entry_set_max_length (CTK_ENTRY (text), field_text->max_len);
			ctk_entry_set_visibility (CTK_ENTRY (text), !field_text->is_password);

			if (txt) {
				ctk_entry_set_text (CTK_ENTRY (text), txt);
				g_free (txt);
			}
			...
			if (txt) {
				ctk_text_buffer_set_text (buffer, txt, -1);
				g_free (txt);
			}
...
static void
ev_view_form_field_choice_changed (CtkWidget   *widget,
				   EvFormField *field)
{
	EvFormFieldChoice *field_choice = EV_FORM_FIELD_CHOICE (field);
	...
	if (CTK_IS_COMBO_BOX (widget)) {
		...
		item = ctk_combo_box_get_active (CTK_COMBO_BOX (widget));
		...
		if (ctk_combo_box_get_has_entry (CTK_COMBO_BOX (widget))) {
			...
			text = ctk_entry_get_text (CTK_ENTRY (ctk_bin_get_child (CTK_BIN (widget))));
	...
	} else if (CTK_IS_TREE_SELECTION (widget)) {
		CtkTreeSelection *selection = CTK_TREE_SELECTION (widget);
		CtkTreeModel     *model;
		GList            *items, *l;

		items = ctk_tree_selection_get_selected_rows (selection, &model);
		g_list_free (field_choice->selected_items);
		field_choice->selected_items = NULL;

		for (l = items; l && l->data; l = g_list_next (l)) {
			CtkTreeIter  iter;
			CtkTreePath *path = (CtkTreePath *)l->data;
			gint         item;
					 ...
					 EvFormField *field)
{
	...
	n_items = ev_document_forms_form_field_choice_get_n_items (EV_DOCUMENT_FORMS (view->document),
								   field);
	model = CTK_TREE_MODEL (ctk_list_store_new (2, G_TYPE_STRING, G_TYPE_INT));
	for (i = 0; i < n_items; i++) {
		CtkTreeIter iter;
		gchar      *item;
	...
	if (field_choice->type == EV_FORM_FIELD_CHOICE_LIST) {
		...
		choice = ctk_scrolled_window_new (NULL, NULL);
		ctk_scrolled_window_set_policy (CTK_SCROLLED_WINDOW (choice),
						CTK_POLICY_AUTOMATIC,
						CTK_POLICY_AUTOMATIC);
		ctk_container_add (CTK_CONTAINER (choice), tree_view);
		ctk_widget_show (tree_view);
		...
		choice = ctk_combo_box_new_with_model_and_entry (model);
		ctk_combo_box_set_entry_text_column (CTK_COMBO_BOX (choice), 0);
		text = ev_document_forms_form_field_choice_get_text (EV_DOCUMENT_FORMS (view->document), field);
		if (text) {
			ctk_entry_set_text (CTK_ENTRY (ctk_bin_get_child (CTK_BIN (choice))), text);
			g_free (text);
		}
		...
		g_signal_connect (choice, "changed",
				  G_CALLBACK (ev_view_form_field_choice_changed),
				  field);
		g_signal_connect_after (ctk_bin_get_child (CTK_BIN (choice)),
					"activate",
					G_CALLBACK (ev_view_form_field_destroy),
					view);
	} else { /* ComboBoxText */
		...
		choice = ctk_combo_box_new_with_model (model);
		renderer = ctk_cell_renderer_text_new ();
		ctk_cell_layout_pack_start (CTK_CELL_LAYOUT (choice),
					    renderer, TRUE);
		ctk_cell_layout_set_attributes (CTK_CELL_LAYOUT (choice),
						renderer,
						"text", 0,
						NULL);
		ctk_combo_box_set_active (CTK_COMBO_BOX (choice), selected_item);
		ctk_combo_box_popup (CTK_COMBO_BOX (choice));
                          ...
                          EvFormField *field)
{
	...
	ctk_widget_show (field_widget);
	ctk_widget_grab_focus (field_widget);
}
			   ...
			   gint               y)
{
	...
	ctk_widget_get_allocation (CTK_WIDGET (view), &allocation);
	ctk_window_get_size (CTK_WINDOW (child->window), &width, &height);

	child->x = x;
	child->y = y;
	ctk_window_move (CTK_WINDOW (child->window),
				       ...
				       CtkWidget *window)
{
	...
	if (child->visible && !ctk_widget_get_visible (window))
...
ev_view_window_children_free (EvView *view)
{
	...
	for (l = view->window_children; l && l->data; l = g_list_next (l)) {
		...
		ctk_widget_destroy (CTK_WIDGET (child->window));
		g_free (child);
	}
...
static void
annotation_window_grab_focus (CtkWidget *widget,
			      EvView    *view)
{
			  ...
			  EvView             *view)
{
	...
	child = ev_view_get_window_child (view, CTK_WIDGET (window));
	child->visible = FALSE;
}
...
ev_view_cancel_add_annotation (EvView *view)
{
	...
	ev_document_misc_get_pointer_position (CTK_WIDGET (view), &x, &y);
                           ...
                           EvAnnotation *annot)
{
        ...
        if (EV_IS_ANNOTATION_MARKUP (annot)) {
                ...
                if (child) {
                        ...
                        ctk_widget_destroy (child->window);
                        g_free (child);
                }
        }
        _ev_view_set_focused_element (view, NULL, -1);
...
{
	CtkSettings *settings = ctk_widget_get_settings (CTK_WIDGET (view));
	gint time;
		       ...
		       CdkRectangle *area)
{
	...
	ctk_style_context_get_style (ctk_widget_get_style_context (CTK_WIDGET (view)),
...
static void
ev_view_size_request (CtkWidget      *widget,
		      CtkRequisition *requisition)
{
...
static void
ev_view_get_preferred_width (CtkWidget *widget,
			     gint *minimum,
			     gint *natural)
{
	CtkRequisition requisition;
...
static void
ev_view_get_preferred_height (CtkWidget *widget,
			      gint *minimum,
			      gint *natural)
{
	CtkRequisition requisition;
...
static void
ev_view_size_allocate (CtkWidget      *widget,
		       CtkAllocation  *allocation)
{
	EvView *view = EV_VIEW (widget);
	GList  *l;
	...
	view_set_adjustment_values (view, CTK_ORIENTATION_HORIZONTAL);
	view_set_adjustment_values (view, CTK_ORIENTATION_VERTICAL);
	...
	for (l = view->children; l && l->data; l = g_list_next (l)) {
		...
		ctk_widget_set_size_request (child->widget, view_area.width, view_area.height);
		ctk_widget_size_allocate (child->widget, &view_area);
	}
...
/* This is based on the deprecated function ctk_draw_insertion_cursor. */
static void
	      ...
	      cairo_t *cr)
{
	...
	if (CTK_WIDGET_CLASS (ev_view_parent_class)->draw)
		(* CTK_WIDGET_CLASS (ev_view_parent_class)->draw) (widget, cr);
...
static gboolean
ev_view_popup_menu (CtkWidget *widget)
{
	gint x, y;
		       ...
		       CtkTooltip *tooltip)
{
	...
	if (annot) {
		...
		if ((contents = ev_annotation_get_contents (annot))) {
			...
			ctk_tooltip_set_tip_area (tooltip, &annot_area);
	...
	if (text && g_utf8_validate (text, -1, NULL)) {
		...
		ctk_tooltip_set_tip_area (tooltip, &link_area);
		g_free (text);
			    ...
			    CdkEventButton *event)
{
	...
	if (ctk_gesture_is_recognized (view->zoom_gesture))
		return TRUE;
	...
	switch (event->button) {
		...
		case 2:
			...
			view->drag_info.hadj = ctk_adjustment_get_value (view->hadjustment);
			view->drag_info.vadj = ctk_adjustment_get_value (view->vadjustment);
...
{
	ctk_container_foreach (CTK_CONTAINER (view), (CtkCallback) ctk_widget_destroy, NULL);
	GList *children, *child;

	children = ctk_container_get_children (CTK_CONTAINER (view));
	for (child = children; child && child->data; child = g_list_next (child)) {
		ctk_container_remove (CTK_CONTAINER (view),
				      CTK_WIDGET (child->data));
	}
...
selection_scroll_timeout_cb (EvView *view)
{
	...
	if (shift)
		ctk_adjustment_set_value (view->vadjustment,
					  CLAMP (ctk_adjustment_get_value (view->vadjustment) + shift,
						 ctk_adjustment_get_lower (view->vadjustment),
						 ctk_adjustment_get_upper (view->vadjustment) -
	...
	if (shift)
		ctk_adjustment_set_value (view->hadjustment,
					  CLAMP (ctk_adjustment_get_value (view->hadjustment) + shift,
						 ctk_adjustment_get_lower (view->hadjustment),
						 ctk_adjustment_get_upper (view->hadjustment) -
						 ctk_adjustment_get_page_size (view->hadjustment)));
...
ev_view_scroll_drag_release (EvView *view)
{
	...
	h_page_size = ctk_adjustment_get_page_size (view->hadjustment);
	v_page_size = ctk_adjustment_get_page_size (view->vadjustment);
	...
	oldhadjustment = ctk_adjustment_get_value (view->hadjustment);
	oldvadjustment = ctk_adjustment_get_value (view->vadjustment);

	h_upper = ctk_adjustment_get_upper (view->hadjustment);
	v_upper = ctk_adjustment_get_upper (view->vadjustment);
			     ...
			     CdkEventMotion *event)
{
	...
	bin_window = ctk_widget_get_window (widget);
	...
	if (view->selection_info.in_drag) {
		...
		if (ctk_drag_check_threshold (widget,
			...
			ctk_target_list_add_uri_targets (target_list, TARGET_DND_URI);
	...
	case 2:
		if (!view->drag_info.in_drag) {
			...
			start = ctk_drag_check_threshold (widget,
		...
		if (view->drag_info.in_drag) {
			...
			dhadj_value = ctk_adjustment_get_page_size (view->hadjustment) *
				      (gdouble)dx / allocation.width;
			dvadj_value = ctk_adjustment_get_page_size (view->vadjustment) *
		     ...
		     gboolean        extend_selections)
{
	...
	if (!clear_selections &&
		...
		ctk_widget_error_bell (CTK_WIDGET (view));
		return TRUE;
	}
			 ...
			 CdkEventKey *event)
{
	...
	/* I expected CTK+ do this for me, but it doesn't cancel
	 * the propagation of bindings handled for the same binding set
	 */
	view->key_binding_handled = FALSE;
	retval = ctk_bindings_activate_event (G_OBJECT (widget), event);
	view->key_binding_handled = FALSE;
...
static void
ev_view_style_updated (CtkWidget *widget)
{
	if (EV_VIEW (widget)->pixbuf_cache)
		ev_pixbuf_cache_style_changed (EV_VIEW (widget)->pixbuf_cache);

	CTK_WIDGET_CLASS (ev_view_parent_class)->style_updated (widget);
}
		 ...
		 gdouble             alpha)
{
	...
	ctk_style_context_save (context);
	ctk_style_context_get_background_color (context, CTK_STATE_FLAG_SELECTED, &color);
	ctk_style_context_restore (context);
                               ...
                               CdkRGBA *fg_color)
{
	...
	state = ctk_widget_has_focus (widget) ? CTK_STATE_FLAG_SELECTED : CTK_STATE_FLAG_ACTIVE;
	       ...
	       gboolean     *page_ready)
{
	...
	*page_ready = TRUE;
	...
	if (ev_document_model_get_inverted_colors (view->model))
		ctk_style_context_add_class (context, EV_STYLE_CLASS_INVERTED);

	if (view->continuous && page == current_page)
		ctk_style_context_set_state (context, CTK_STATE_FLAG_ACTIVE);

	ctk_render_background (context, cr, page_area->x, page_area->y, page_area->width, page_area->height);
		      ...
		      GParamSpec   *pspec)
{
	...
	case PROP_IS_LOADING:
		...
		break;
	case PROP_HADJUSTMENT:
		set_scroll_adjustment (view, CTK_ORIENTATION_HORIZONTAL,
				       (CtkAdjustment *) g_value_get_object (value));
		break;
	case PROP_VADJUSTMENT:
		set_scroll_adjustment (view, CTK_ORIENTATION_VERTICAL,
				       (CtkAdjustment *) g_value_get_object (value));
		break;
	case PROP_HSCROLL_POLICY:
...
/* Accessibility */
static AtkObject *
ev_view_get_accessible (CtkWidget *widget)
{
	EvView *view = EV_VIEW (widget);
...
/* CtkContainer */
static void
ev_view_remove (CtkContainer *container,
		CtkWidget    *widget)
{
...
static void
pan_gesture_pan_cb (CtkGesturePan   *gesture,
...
#define PAN_ACTION_DISTANCE 200
	...
	if (offset > PAN_ACTION_DISTANCE) {
		if (direction == CTK_PAN_DIRECTION_LEFT ||
		    ctk_widget_get_direction (CTK_WIDGET (view)) == CTK_TEXT_DIR_RTL)
			   ...
			   CtkWidget *previous_toplevel)
{
	...
	if (parent && !view->pan_gesture) {
		view->pan_gesture =
			ctk_gesture_pan_new (parent, CTK_ORIENTATION_HORIZONTAL);
		...
		ctk_gesture_single_set_touch_only (CTK_GESTURE_SINGLE (view->pan_gesture), TRUE);
		ctk_event_controller_set_propagation_phase (CTK_EVENT_CONTROLLER (view->pan_gesture),
							    CTK_PHASE_CAPTURE);
		    ...
		    CtkWidget *previous_parent)
{
	...
	g_assert (!parent || CTK_IS_SCROLLED_WINDOW (parent));
}
...
ev_view_class_init (EvViewClass *class)
{
	...
	ctk_widget_class_set_css_name (widget_class, "evview");
...
static void
zoom_gesture_scale_changed_cb (CtkGestureZoom *gesture,
			       gdouble         scale,
			       EvView         *view)
{
	gdouble factor;
	...
	ctk_gesture_get_bounding_box_center (CTK_GESTURE (gesture), &view->zoom_center_x, &view->zoom_center_y);
...
ev_view_init (EvView *view)
{
	...
	ctk_widget_set_can_focus (CTK_WIDGET (view), TRUE);
	ctk_widget_set_has_window (CTK_WIDGET (view), TRUE);
	ctk_widget_set_redraw_on_allocate (CTK_WIDGET (view), FALSE);

	context = ctk_widget_get_style_context (CTK_WIDGET (view));
	ctk_style_context_add_class (context, "content-view");
	...
	view->zoom_gesture = ctk_gesture_zoom_new (CTK_WIDGET (view));
	ctk_event_controller_set_propagation_phase (CTK_EVENT_CONTROLLER (view->zoom_gesture),
						    CTK_PHASE_CAPTURE);
			     ...
			     EvView        *view)
{
	...
	if (view->hadjustment) {
		value = (gint) ctk_adjustment_get_value (view->hadjustment);
	...
	if (view->vadjustment) {
		value = (gint) ctk_adjustment_get_value (view->vadjustment);
	...
	for (l = view->children; l && l->data; l = g_list_next (l)) {
		...
		child->x += dx;
		child->y += dy;
		if (ctk_widget_get_visible (child->widget) && ctk_widget_get_visible (widget))
...
setup_caches (EvView *view)
{
	...
	view->pixbuf_cache = ev_pixbuf_cache_new (CTK_WIDGET (view), view->model, view->pixbuf_cache_size);
...
ev_view_autoscroll_cb (EvView *view)
{
	...
	value = ctk_adjustment_get_value (view->vadjustment);
						...
						int     height)
{
	...
	sb_size = ev_view_get_scrollbar_size (view, CTK_ORIENTATION_VERTICAL);
	...
	case EV_SIZING_FIT_PAGE:
		...
		break;
	case EV_SIZING_AUTOMATIC:
		scale = zoom_for_size_automatic (ctk_widget_get_screen (CTK_WIDGET (view)),
		   ...
		   CtkOrientation  orientation)
{
	...
	ctk_widget_get_allocation (CTK_WIDGET (view), &allocation);
	ev_view_size_request (CTK_WIDGET (view), &requisition);
			...
			gpointer          data)
{
	...
	if (ev_view->link_selected) {
		...
		text = get_selected_text (ev_view);
		if (text) {
			ctk_selection_data_set_text (selection_data, text, -1);
			g_free (text);
		}
	}
}
...
ev_view_update_primary_selection (EvView *ev_view)
{
	...
	if (ev_view->selection_info.selections || ev_view->link_selected) {
		...
		if (!ctk_clipboard_set_with_owner (clipboard,
						   targets, n_targets,
		...
		if (ctk_clipboard_get_owner (clipboard) == G_OBJECT (ev_view))
...
ev_view_set_cursor (EvView *view, EvViewCursor new_cursor)
{
	...
	bin_window = ctk_widget_get_window (CTK_WIDGET (view));
